...
assert_readable_panic() {
  file=$1
  expected=$2
  assert_contains "$file" "$expected"
  assert_contains "$file" "[JAVAN-RUNTIME-PANIC]"
  assert_contains "$file" "Where:"
  assert_contains "$file" "Why:"
...
accepts_readable_native_panic() {
  ...
  set -e
  if [ "$exit_code" -eq 0 ]; then
    fail "$project readable native panic unexpectedly succeeded"
  fi
  if [ "$exit_code" -ne 1 ]; then
    printf '%s\n' "Expected $project readable native panic exit 1, got $exit_code" >&2
    exit 1
  fi
  assert_empty "$TMP/$build_name.native"
  assert_readable_panic "$TMP/$build_name.native.err" "$expected"
  pass "$project readable native panic is deterministic"
}
...
accepts_readable_native_panic_envs() {
  project=$1
  expected=$2
  shift 2
  name=$(basename "$project")
  ...
  assert_empty "$TMP/$build_name.native"
  assert_readable_panic "$TMP/$build_name.native.err" "$expected"
  pass "$project readable native panic is deterministic with env set"
}
...
accepts_runtime_contract_report() {
  ...
  assert_contains "$full_project/.javan/reports/runtime.json" "\"javaAllocationOwnership\": \"javan-owned-generated-objects-object-arrays-primitive-arrays-boxed-primitive-wrappers-runtime-strings-runtime-containers-and-owned-container-storage-gc-eligible\""
  assert_contains "$full_project/.javan/reports/runtime.json" "\"ffiAllocationOwnership\": \"caller-frees-javan-owned-strings-and-byte-arrays-with-javan_free-result-diagnostics-with-javan_result_free\""
  ...
  assert_contains "$full_project/.javan/reports/runtime.json" "\"heapReclamationScope\": \"generated-objects-object-arrays-primitive-arrays-boxed-primitive-wrappers-runtime-strings-runtime-containers-and-owned-container-storage\""
  ...
  assert_contains "$full_project/.javan/reports/runtime.json" "\"ownedBufferReferenceValidation\": true"
  assert_contains "$full_project/.javan/reports/runtime.json" "\"ownedBufferReferenceValidationScope\": \"list-map-stringbuilder-owned-backing-storage\""
  ...
  assert_contains "$full_project/.javan/reports/runtime.json" "\"operandCallTemporaryRootScope\": [\"object-call-arguments\", \"nested-object-call-results\", \"field-store-receiver\", \"field-store-value\", \"array-store-array\", \"array-store-value\", \"return-operand\", \"object-print-operands\", \"object-compare-operands\", \"field-load-receiver\", \"field-load-result\", \"chained-field-load-receivers\", \"chained-call-receivers\"]"
  ...
  assert_contains "$full_project/.javan/reports/runtime.json" "\"allocationPathCollectionScope\": \"generated-objects-object-arrays-primitive-arrays-boxed-primitive-wrappers-runtime-strings-runtime-containers-and-owned-container-storage\""
...
accepts_native_library() {
  ...
  full_project="$ROOT/$project"
  "$JAVAN_BIN" clean "$full_project" >/dev/null 2>&1 || true
  run_cmd "native-library-build" "$JAVAN_BIN" build "$full_project" --library --format static --export com.acme.Math.add --export com.acme.Text.greet --export com.acme.Bytes.duplicate --export com.acme.Bytes.merge --export com.acme.Store.rememberString --export com.acme.Store.lastString --export com.acme.Store.rememberBytes --export com.acme.Store.lastBytes --export com.acme.Store.clear --export com.acme.Failures.failInt --bindings c,rust,go,python
  ...
  assert_contains "$full_project/.javan/reports/runtime.json" "\"javan_export_com_acme_Bytes_merge_bytes_bytes\""
  assert_contains "$full_project/.javan/reports/runtime.json" "\"javan_export_com_acme_Store_rememberString_string\""
  assert_contains "$full_project/.javan/reports/runtime.json" "\"javan_export_com_acme_Store_lastString_void\""
  assert_contains "$full_project/.javan/reports/runtime.json" "\"javan_export_com_acme_Failures_failInt_void\""
...
accepts_jvm_equivalent_app_envs "$NATIVE_PROFILE_PROJECTS/boxed-integer-gc" com.acme.Main \
  JAVAN_HEAP_LIMIT_BYTES=4096 \
  JAVAN_GC_STRESS=1 \
  JAVAN_GC_SAFEPOINT_INTERVAL=1
accepts_jvm_equivalent_app_envs "$NATIVE_PROFILE_PROJECTS/boxed-boolean-gc" com.acme.Main \
  JAVAN_HEAP_LIMIT_BYTES=4096 \
  JAVAN_GC_STRESS=1 \
  JAVAN_GC_SAFEPOINT_INTERVAL=1
accepts_jvm_equivalent_app_envs "$NATIVE_PROFILE_PROJECTS/runtime-filetime-gc" com.acme.Main \
  JAVAN_HEAP_LIMIT_BYTES=8192 \
  JAVAN_GC_STRESS=1 \
  JAVAN_GC_SAFEPOINT_INTERVAL=1
accepts_jvm_equivalent_app_envs "$NATIVE_PROFILE_PROJECTS/runtime-duration-millis-gc" com.acme.Main \
  JAVAN_HEAP_LIMIT_BYTES=8192 \
  JAVAN_GC_STRESS=1 \
  JAVAN_GC_SAFEPOINT_INTERVAL=1
accepts_jvm_equivalent_app_envs "$NATIVE_PROFILE_PROJECTS/runtime-duration-seconds-gc" com.acme.Main \
  JAVAN_HEAP_LIMIT_BYTES=8192 \
  JAVAN_GC_STRESS=1 \
  JAVAN_GC_SAFEPOINT_INTERVAL=1
accepts_jvm_equivalent_app_envs "$NATIVE_PROFILE_PROJECTS/boxed-long-gc" com.acme.Main \
  JAVAN_HEAP_LIMIT_BYTES=8192 \
  JAVAN_GC_STRESS=1 \
  JAVAN_GC_SAFEPOINT_INTERVAL=1
accepts_jvm_equivalent_app_envs "$NATIVE_PROFILE_PROJECTS/boxed-float-gc" com.acme.Main \
  JAVAN_HEAP_LIMIT_BYTES=4096 \
  JAVAN_GC_STRESS=1 \
  JAVAN_GC_SAFEPOINT_INTERVAL=1
accepts_jvm_equivalent_app_envs "$NATIVE_PROFILE_PROJECTS/boxed-double-gc" com.acme.Main \
  JAVAN_HEAP_LIMIT_BYTES=8192 \
  JAVAN_GC_STRESS=1 \
  JAVAN_GC_SAFEPOINT_INTERVAL=1
...
accepts_jvm_equivalent_app_envs "$NATIVE_PROFILE_PROJECTS/runtime-map-realloc-gc" com.acme.Main \
  JAVAN_HEAP_LIMIT_BYTES=8192 \
  JAVAN_GC_STRESS=1 \
  JAVAN_GC_SAFEPOINT_INTERVAL=1
...
accepts_jvm_equivalent_app_envs "$NATIVE_PROFILE_PROJECTS/runtime-list-of-varargs-gc" com.acme.Main \
  JAVAN_HEAP_LIMIT_BYTES=4096 \
  JAVAN_GC_STRESS=1 \
  JAVAN_GC_SAFEPOINT_INTERVAL=1
...
accepts_jvm_equivalent_app_envs "$NATIVE_PROFILE_PROJECTS/operand-array-load-temporary-root" com.acme.Main \
  ...
  JAVAN_GC_STRESS=1 \
  JAVAN_GC_SAFEPOINT_INTERVAL=1
accepts_jvm_equivalent_app_envs "$NATIVE_PROFILE_PROJECTS/operand-object-compare-temporary-root" com.acme.Main \
  JAVAN_HEAP_LIMIT_BYTES=4096 \
  JAVAN_GC_STRESS=1 \
  JAVAN_GC_SAFEPOINT_INTERVAL=1
accepts_jvm_equivalent_app_envs "$NATIVE_PROFILE_PROJECTS/operand-field-load-temporary-root" com.acme.Main \
  JAVAN_HEAP_LIMIT_BYTES=4096 \
  JAVAN_GC_STRESS=1 \
  JAVAN_GC_SAFEPOINT_INTERVAL=1
accepts_jvm_equivalent_app_envs "$NATIVE_PROFILE_PROJECTS/operand-chained-field-load-temporary-root" com.acme.Main \
  JAVAN_HEAP_LIMIT_BYTES=4096 \
  JAVAN_GC_STRESS=1 \
  JAVAN_GC_SAFEPOINT_INTERVAL=1
accepts_jvm_equivalent_app_envs "$NATIVE_PROFILE_PROJECTS/operand-chained-call-receiver-temporary-root" com.acme.Main \
  JAVAN_HEAP_LIMIT_BYTES=4096 \
  JAVAN_GC_STRESS=1 \
  JAVAN_GC_SAFEPOINT_INTERVAL=1
...
accepts_jvm_equivalent_app_envs "$NATIVE_PROFILE_PROJECTS/local-root-liveness-gc" com.acme.Main \
  JAVAN_HEAP_LIMIT_BYTES=6000 \
  JAVAN_GC_STRESS=1 \
  JAVAN_GC_SAFEPOINT_INTERVAL=1
accepts_jvm_equivalent_app_envs "$NATIVE_PROFILE_PROJECTS/cfg-local-root-liveness-gc" com.acme.Main \
  JAVAN_HEAP_LIMIT_BYTES=6000 \
  JAVAN_GC_STRESS=1 \
  JAVAN_GC_SAFEPOINT_INTERVAL=1
...
accepts_readable_native_panic "$NATIVE_PROFILE_PROJECTS/exception-panic" "boom"
accepts_readable_native_panic "$NATIVE_PROFILE_PROJECTS/exception-default-panic" "javan panic"
accepts_readable_native_panic_envs "$NATIVE_PROFILE_PROJECTS/panic-string-concat-temporary-root" "left-right" \
  JAVAN_HEAP_LIMIT_BYTES=4096 \
  JAVAN_GC_STRESS=1 \
  JAVAN_GC_SAFEPOINT_INTERVAL=1
...
accepts_readable_native_panic_envs "$NATIVE_PROFILE_PROJECTS/runtime-list-allocation-limit-panic" "out of memory" JAVAN_MAX_ALLOCATION_BYTES=48
accepts_readable_native_panic_envs "$NATIVE_PROFILE_PROJECTS/runtime-map-allocation-limit-panic" "out of memory" JAVAN_MAX_ALLOCATION_BYTES=96
accepts_readable_native_panic_envs "$NATIVE_PROFILE_PROJECTS/runtime-path-allocation-limit-panic" "out of memory" JAVAN_MAX_ALLOCATION_BYTES=128
accepts_readable_native_panic_envs "$NATIVE_PROFILE_PROJECTS/runtime-read-string-allocation-limit-panic" "out of memory" JAVAN_MAX_ALLOCATION_BYTES=1024
accepts_readable_native_panic_envs "$NATIVE_PROFILE_PROJECTS/runtime-read-all-bytes-allocation-limit-panic" "out of memory" JAVAN_MAX_ALLOCATION_BYTES=512
accepts_readable_native_panic_envs "$NATIVE_PROFILE_PROJECTS/runtime-directory-stream-child-allocation-limit-panic" "out of memory" JAVAN_MAX_ALLOCATION_BYTES=256
accepts_readable_native_panic_envs "$NATIVE_PROFILE_PROJECTS/runtime-process-run-output-allocation-limit-panic" "out of memory" JAVAN_MAX_ALLOCATION_BYTES=512
accepts_readable_native_panic "$NATIVE_PROFILE_PROJECTS/runtime-stringbuilder-setlength-overflow-panic" "string builder length overflow"
accepts_readable_native_panic_envs "$NATIVE_PROFILE_PROJECTS/array-copy-allocation-limit-panic" "out of memory" JAVAN_MAX_ALLOCATION_BYTES=128
accepts_readable_native_panic_envs "$NATIVE_PROFILE_PROJECTS/heap-limit-live-root-panic" "out of memory" \
...
rejects_check "$NEGATIVE_PROJECTS/unsupported-http-client" "error[JAVAN061]"
