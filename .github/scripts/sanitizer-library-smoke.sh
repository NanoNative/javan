...
configure_asan_options "$SANITIZER_REQUIRED" "required sanitizer library run"
...
EOF
  ...
  set +e
  LD_LIBRARY_PATH="$(dirname "$SHARED_ARTIFACT")${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}"
  DYLD_LIBRARY_PATH="$(dirname "$SHARED_ARTIFACT")${DYLD_LIBRARY_PATH:+:$DYLD_LIBRARY_PATH}"
