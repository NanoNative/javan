...
final class RuntimeSourceMemorySections {
    private static final String SOURCE_HEAP_HEAD = """
        ...
        #define JAVAN_HTTP_HEADERS_MAGIC 0x4a485848
        #define JAVAN_HTTP_EXCHANGE_INPUT_STREAM_MAGIC 0x4a485849
        ...
        static JAVAN_THREAD_LOCAL int javan_utf16_cursor_cache_pending_low_surrogate = -1;
    ...
    private static final String SOURCE_HEAP_TAIL_A = """
        ...
        static void javan_validate_runtime_container_references(javan_allocation_node* node) {
            ...
            if (node->runtime_kind == JAVAN_RUNTIME_KIND_OBJECT_LIST) {
                ...
                if (builder->values != NULL && builder->values[builder->byte_length] != '\\0') {
            ...
            } else if (node->runtime_kind == JAVAN_RUNTIME_KIND_UUID) {
                javan_uuid_state* state = (javan_uuid_state*) node->value;
                if (state->magic != JAVAN_UUID_MAGIC
                    || (state->bytes[6] & 0xf0U) != 0x40U
                    || (state->bytes[8] & 0xc0U) != 0x80U) {
                    javan_panic("invalid runtime UUID metadata");
                }
                ...
                javan_byte_input_stream_value* stream = (javan_byte_input_stream_value*) node->value;
                ...
                javan_validate_runtime_managed_reference(exchange->request_headers);
            ...
            } else if (node->runtime_kind == JAVAN_RUNTIME_KIND_HTTP_HEADERS) {
                javan_http_headers_value* headers = (javan_http_headers_value*) node->value;
                if (headers->magic != JAVAN_HTTP_HEADERS_MAGIC || headers->request == NULL) {
                    javan_panic("invalid runtime http headers metadata");
                }
                javan_validate_runtime_managed_reference(headers->request);
            } else if (node->runtime_kind == JAVAN_RUNTIME_KIND_HTTP_EXCHANGE_INPUT_STREAM) {
                javan_byte_input_stream_value* stream = (javan_byte_input_stream_value*) node->value;
                if (stream->magic != JAVAN_HTTP_EXCHANGE_INPUT_STREAM_MAGIC
                    || stream->bytes == NULL
                    || stream->length < 0
                    || stream->position < 0
                    || stream->position > stream->length) {
                    javan_panic("invalid runtime http request body metadata");
                }
        ...
        void* javan_printable_object_string(void* value) {
            ...
            if (snapshot.runtime_kind == JAVAN_RUNTIME_KIND_UUID) {
    ...
    private static final String SOURCE_HEAP_ALLOC_HEAD = """
        static void* javan_alloc_with_root_and_kind(unsigned long size, void** root_slot, int runtime_kind) {
            javan_runtime_lock_enter();
        ...
        static void* javan_alloc_with_root(unsigned long size, void** root_slot) {
            return javan_alloc_with_root_and_kind(size, root_slot, JAVAN_RUNTIME_KIND_NONE);
        }

        void* javan_alloc(unsigned long size) {
            return javan_alloc_with_root(size, NULL);
        }

        static void* javan_alloc_rooted(unsigned long size, void** root_slot) {
            if (root_slot == NULL) {
                javan_panic("missing runtime allocation root");
            }
            return javan_alloc_with_root(size, root_slot);
        }

        static void* javan_alloc_runtime_rooted(unsigned long size, void** root_slot, int runtime_kind) {
            if (root_slot == NULL) {
                javan_panic("missing runtime allocation root");
            }
            if (runtime_kind == JAVAN_RUNTIME_KIND_NONE) {
                javan_panic("missing runtime allocation kind");
            }
            return javan_alloc_with_root_and_kind(size, root_slot, runtime_kind);
        }
        ...
        static void* javan_export_alloc(unsigned long size) {
            ...
            if (javan_allocator_cleaning == 0) {
                javan_track_allocation(
                    value,
                    (void*) header,
                    actual_size,
                    JAVAN_HEAP_KIND_EXPORT,
                    0,
                    JAVAN_RUNTIME_KIND_NONE
                );
            }
            javan_runtime_lock_leave();
            return value;
    ...
    private static final String SOURCE_HEAP_TAIL_C = """
        ...
        typedef struct {
            ...
            int pending_throwable_line;
            int pending_throwable_bytecode_offset;
            const char* pending_throwable_source_line;
        } javan_throwable_state;
        ...
        typedef struct {
            ...
            int throwable_line;
            int throwable_bytecode_offset;
            const char* throwable_source_line;
        } javan_caught_throwable;
        ...
        typedef struct javan_thread {
            ...
            #endif
            javan_throwable_state* throwable_state;
            void* target;
    ...
    private static final String SOURCE_HEAP_ALLOC_TAIL = """
        ...
        static void javan_pending_clear_state(javan_throwable_state* state) {
            ...
            state->pending_throwable_line = -1;
            state->pending_throwable_bytecode_offset = -1;
            state->pending_throwable_source_line = NULL;
        }

        static javan_throwable_state* javan_thread_throwable_state(javan_thread* thread) {
            if (thread == NULL) {
                javan_panic("missing throwable state thread");
            }
            if (thread->throwable_state != NULL) {
                return thread->throwable_state;
            }
            ...
            javan_alloc_runtime_rooted(
                ...
                &rooted_allocated,
                JAVAN_RUNTIME_KIND_THROWABLE_STATE
            );
            ...
            javan_throwable_state* result = thread->throwable_state;
            javan_runtime_lock_leave();
            javan_root_frame_pop(roots);
            return result;
        }
        ...
        void javan_pending_throw(
            ...
            if (throwable_type == NULL || throwable_type[0] == '\\0') {
                javan_panic("invalid pending throwable type");
            }
            javan_thread* thread = javan_current_thread_object();
    ...
    private static final String SOURCE_HEAP_ALLOC_EXCEPTIONS = """
        ...
        int javan_pending_type_is(void* throwable_type) {
            ...
            javan_throwable_state* state = ((javan_thread*) javan_current_thread_value)->throwable_state;
            const char* actual = state == NULL ? NULL : state->pending_throwable_type;
        ...
        static javan_caught_throwable* javan_require_caught_throwable(void* value) {
            if (value == NULL) {
        ...
        void javan_pending_catch_into(void** result) {
            ...
            *result = NULL;
            ...
            javan_caught_throwable* caught = (javan_caught_throwable*) rooted_caught;
            javan_runtime_lock_enter();
            javan_throwable_state* state = thread->throwable_state;
            if (state == NULL || state->pending_throwable_type == NULL) {
                javan_runtime_lock_leave();
                javan_root_frame_pop(roots);
                javan_panic("missing pending throwable");
            }
            ...
            caught->throwable_line = state->pending_throwable_line;
        ...
        void javan_pending_rethrow(void* value) {
            ...
            state->pending_throwable_line = caught->throwable_line;
            state->pending_throwable_bytecode_offset = caught->throwable_bytecode_offset;
            state->pending_throwable_source_line = caught->throwable_source_line;
            javan_runtime_lock_leave();
        }
        ...
        void javan_pending_panic(void) {
            ...
            const char* class_name = state->pending_throwable_class;
            const char* method = state->pending_throwable_method;
            const char* file = state->pending_throwable_file;
            const int line = state->pending_throwable_line;
            const int bytecode_offset = state->pending_throwable_bytecode_offset;
            const char* source_line = state->pending_throwable_source_line;
            const char* detail_source = state->pending_throwable_message == NULL
                ? "javan panic"
                : (const char*) state->pending_throwable_message;
            char summary_copy[256];
            snprintf(
                summary_copy,
                sizeof(summary_copy),
                "uncaught Java exception (%s)",
                state->pending_throwable_type
            );
            ...
            javan_panic_at(
                ...
                "Catch it in Java or let the application terminate intentionally.",
                detail_copy
            );
        }
        ...
        static void javan_gc_mark_runtime_children(void* value, int runtime_kind) {
            ...
            if (runtime_kind == JAVAN_RUNTIME_KIND_OBJECT_LIST) {
                ...
                if (exchange != NULL && exchange->magic == JAVAN_HTTP_EXCHANGE_MAGIC) {
                    ...
                    javan_gc_mark_value(exchange->request_headers);
                    javan_gc_mark_value(exchange->request_body);
                    javan_gc_mark_value(exchange->response_body);
                }
            } else if (runtime_kind == JAVAN_RUNTIME_KIND_HTTP_HEADERS) {
                javan_http_headers_value* headers = (javan_http_headers_value*) value;
                if (headers != NULL && headers->magic == JAVAN_HTTP_HEADERS_MAGIC) {
                    javan_gc_mark_value(headers->request);
                }
            } else if (runtime_kind == JAVAN_RUNTIME_KIND_HTTP_EXCHANGE_INPUT_STREAM) {
                javan_byte_input_stream_value* stream = (javan_byte_input_stream_value*) value;
                if (stream != NULL && stream->magic == JAVAN_HTTP_EXCHANGE_INPUT_STREAM_MAGIC) {
                    javan_gc_mark_value(stream->bytes);
                }
    ...
    private static final String SOURCE_ARRAYS = """
        ...
        static void javan_os_random_bytes(unsigned char* target, int length) {
            if (length < 0 || (length > 0 && target == NULL)) {
                javan_panic("invalid OS entropy target");
            }
            if (length == 0) {
                return;
            }
            ...
            if (library == NULL) {
                javan_panic("could not load Windows system entropy");
            }
            ...
            if (generate == NULL) {
                FreeLibrary(library);
                javan_panic("could not resolve Windows system entropy");
            }
            LONG status = generate(NULL, target, (ULONG) length, 0x00000002UL);
            FreeLibrary(library);
            if (status < 0) {
                javan_panic("could not read Windows system entropy");
            }
        #else
            int descriptor;
            do {
            ...
            if (descriptor < 0) {
                javan_panic("could not open OS entropy");
            }
            int offset = 0;
            while (offset < length) {
                ssize_t count = read(descriptor, target + offset, (size_t) (length - offset));
                if (count > 0) {
                    offset += (int) count;
                    ...
                    close(descriptor);
                    javan_panic("could not read OS entropy");
                }
            }
            close(descriptor);
        #endif
        ...
        void javan_secure_random_next_bytes(void* value, void* array) {
            ...
            javan_os_random_bytes((unsigned char*) bytes->values, bytes->length);
        }

        void* javan_uuid_random(void) {
            javan_uuid_state* uuid = (javan_uuid_state*) javan_alloc(sizeof(javan_uuid_state));
            uuid->magic = JAVAN_UUID_MAGIC;
            uuid->reserved0 = 0;
            javan_os_random_bytes(uuid->bytes, 16);
            uuid->bytes[6] = (unsigned char) ((uuid->bytes[6] & 0x0fU) | 0x40U);
        ...
        void* javan_uuid_to_string(void* value) {
            if (value == NULL || ((javan_uuid_state*) value)->magic != JAVAN_UUID_MAGIC) {
                javan_panic("invalid UUID state");
            }
            static const char hex[] = "0123456789abcdef";
            void* uuid_root = value;
            void** roots[] = {(void**) &uuid_root};
            javan_root_frame_push(roots, 1);
            javan_uuid_state* uuid = (javan_uuid_state*) uuid_root;
            char* text = javan_string_alloc(37);
            int output = 0;
            for (int index = 0; index < 16; index++) {
                unsigned char byte = uuid->bytes[index];
                text[output++] = hex[byte >> 4];
                text[output++] = hex[byte & 0x0fU];
    ...
    private static final String SOURCE_ARRAYS_TAIL = """
        ...
        static int javan_utf16_next_char(const unsigned char** cursor, int* pending_low_surrogate) {
            if (*pending_low_surrogate >= 0) {
                int value = *pending_low_surrogate;
                *pending_low_surrogate = -1;
                return value;
            }
            unsigned int code_point = javan_utf8_next_code_point(cursor, 1);
            if (code_point <= 0xFFFFU) {
                return (int) code_point;
            }
            unsigned int adjusted = code_point - 0x10000U;
            *pending_low_surrogate = (int) (0xDC00U + (adjusted & 0x3FFU));
        ...
        static int javan_utf16_char_at_utf8(const char* value, int index) {
            ...
            const unsigned char* current = (const unsigned char*) value;
            int current_index = 0;
            int pending_low_surrogate = -1;
            if (value == javan_utf16_cursor_cache_value && index >= javan_utf16_cursor_cache_index) {
                current = javan_utf16_cursor_cache_position;
                current_index = javan_utf16_cursor_cache_index;
                pending_low_surrogate = javan_utf16_cursor_cache_pending_low_surrogate;
            }
            while (*current != 0U || pending_low_surrogate >= 0) {
                int character = javan_utf16_next_char(&current, &pending_low_surrogate);
                if (current_index == index) {
                    javan_utf16_cursor_cache_value = value;
                    javan_utf16_cursor_cache_position = current;
                    javan_utf16_cursor_cache_index = current_index + 1;
                    javan_utf16_cursor_cache_pending_low_surrogate = pending_low_surrogate;
                    return character;
                }
                current_index++;
            }
        ...
        static void javan_utf16_cursor_cache_invalidate(const char* value) {
            if (value == javan_utf16_cursor_cache_value) {
                javan_utf16_cursor_cache_value = NULL;
                javan_utf16_cursor_cache_position = NULL;
                javan_utf16_cursor_cache_index = 0;
                javan_utf16_cursor_cache_pending_low_surrogate = -1;
            }
        }

        static int javan_string_has_standard_utf8_code_point(const char* value) {
            const unsigned char* current = (const unsigned char*) value;
            while (*current != 0U) {
        ...
        static int javan_utf16_byte_offset(const char* value, int index) {
            if (index < 0) {
                javan_panic("string index out of bounds");
            }
            const unsigned char* start = (const unsigned char*) value;
            ...
            while (*current != 0U) {
                ...
                (void) javan_utf8_next_code_point(&current, 1);
                current_index++;
            }
            if (current_index == index) {
                return (int) (current - start);
        ...
        static int javan_utf16_length_to_byte_offset(const char* value, const char* end) {
            const unsigned char* current = (const unsigned char*) value;
            ...
            if (current != limit) {
                javan_panic("invalid UTF-8 string boundary");
            }
            return length;
        }
        ...
        int javan_string_index_of_string_from(const char* value, const char* needle, int from_index) {
            ...
            if (javan_string_has_standard_utf8_code_point(value) == 0
                && javan_string_has_standard_utf8_code_point(needle) == 0) {
                int start_offset = javan_utf16_byte_offset(value, start);
                const char* match = strstr(value + start_offset, needle);
                return match == NULL ? -1 : javan_utf16_length_to_byte_offset(value, match);
            }
        ...
        static void* javan_string_copy_utf16_range(const char* value, int begin, int end) {
            ...
            for (int index = 0; index < end; index++) {
                int character = javan_utf16_next_char(&current, &pending_low_surrogate);
                if (index >= begin) {
                    int char_length = javan_modified_utf8_char_length(character);
            ...
            char* out = result;
            current = (const unsigned char*) source_root;
            pending_low_surrogate = -1;
            for (int index = 0; index < end; index++) {
                int character = javan_utf16_next_char(&current, &pending_low_surrogate);
                if (index >= begin) {
                    out = javan_modified_utf8_write_char(out, character);
                }
            }
            *out = '\\0';
        ...
        void* javan_string_replace_char(const char* value, int old_ch, int new_ch) {
            ...
            while (*current != 0U || pending_low_surrogate >= 0) {
                int character = javan_utf16_next_char(&current, &pending_low_surrogate);
                int replacement = character == (old_ch & 0xffff) ? new_ch : character;
            ...
            while (*current != 0U || pending_low_surrogate >= 0) {
                int character = javan_utf16_next_char(&current, &pending_low_surrogate);
                out = javan_modified_utf8_write_char(
                    out,
                    character == (old_ch & 0xffff) ? new_ch : character
                );
            }
            *out = '\\0';
    ...
    private static final String SOURCE_COLLECTIONS_TAIL = """
        ...
        static void javan_string_validate_strict_utf8(const char* value) {
            const unsigned char* current = (const unsigned char*) value;
            while (*current != 0U) {
                (void) javan_utf8_next_code_point(&current, 0);
            }
        }
        ...
        void* javan_string_from(const char* value) {
            ...
            javan_string_validate_strict_utf8(value);
            return javan_string_copy(value);
        }
    ...
    private static String platformThrowableHierarchy() {
        final StringBuilder source = new StringBuilder("""
            static const char* javan_platform_throwable_parent(const char* throwable_type) {
                if (throwable_type == NULL || strcmp(throwable_type, "java/lang/Throwable") == 0) {
                    return "";
                }
            """);
        for (final JdkCallSupport.PlatformThrowableParent edge : JdkCallSupport.platformThrowableParents()) {
            source.append("    if (strcmp(throwable_type, \"")
                .append(edge.type())
                .append("\") == 0) {\n")
                .append("        return \"")
                .append(edge.parent())
