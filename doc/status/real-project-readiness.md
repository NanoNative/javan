...
| Nano HTTP service | In progress | A stripped real Nano `/hello` service route without `DevConsoleService` now gets past inherited `Nano.subscribeEvent(...)->NanoBase` resolution and inherited JDK `Context.computeIfAbsent(..., Function)` classification. The current deterministic blocker set is now pinned in tests and reports: `JAVAN012` on `FunctionOrNull.apply(Object)`, repeated `JAVAN030` lambda `invokedynamic`, `JAVAN014` exception handlers, and unsupported JDK calls such as `Object.getClass()`. | Closed-world interface/lambda dispatch for `FunctionOrNull` plus broader lambda `invokedynamic`, exception lowering, and missing JDK substitutions in the Nano bootstrap chain, then broader HTTP service runtime, resources, thread/blocking model, and dev-console/reflection exclusion. |
...
- stripped real Nano `/hello` service route now gets past inherited `Nano.subscribeEvent(...)->NanoBase` resolution and inherited JDK `Context.computeIfAbsent(..., Function)` classification, and its current deterministic blocker set is pinned in reports: `FunctionOrNull.apply(Object)`, repeated lambda `invokedynamic`, unsupported exception handlers, and missing JDK substitutions in the Nano bootstrap chain before HTTP runtime behavior can be claimed
...
6. in progress: stripped Nano `/hello` route without `DevConsoleService` now reproduces the current blocker set deterministically; inherited `Nano.subscribeEvent(...)->NanoBase` resolution and inherited JDK `Context.computeIfAbsent(..., Function)` classification are fixed locally, and the current pinned gates are `FunctionOrNull.apply(Object)`, repeated lambda `invokedynamic`, unsupported exception handlers, and missing JDK substitutions before the HTTP runtime
