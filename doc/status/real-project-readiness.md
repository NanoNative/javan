...
- the current main-based Nano frontier has moved through the first scheduler/bootstrap worker slices: `ScheduledThreadPoolExecutor.<init>(int)`, `ScheduledThreadPoolExecutor.<init>(int,ThreadFactory,RejectedExecutionHandler)`, same-method static virtual-thread builder/factory bootstrap, runtime ownership for `Scheduler extends ScheduledThreadPoolExecutor`, `AtomicLong(long|get|incrementAndGet|decrementAndGet)`, `ExecutorService.submit(Runnable)`, and `Future.cancel(boolean)` are now integrated and focused-tested; the next concrete Nano blocker is scheduled work lifecycle support such as `schedule(...)`, `scheduleAtFixedRate(...)`, `awaitTermination(...)`, and `shutdownNow()`
