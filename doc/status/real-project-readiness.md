...
| Nano HTTP service | In progress | A stripped real Nano `/hello` service route without `DevConsoleService` now gets past inherited `Nano.subscribeEvent(...)->NanoBase` resolution, inherited JDK `Context.computeIfAbsent(..., Function)` classification, inherited TypeMap default-interface false positives, the hidden-lambda default wrapper false positive on `FunctionOrNull.apply(Object)`, and the inherited-JDK `Context.put(...)->ConcurrentTypeMap.put(...)` false positive. The current deterministic blocker set is now pinned in tests and reports: `JAVAN012` on `FunctionOrNull.applyWithException(Object)` and `ExRunnable.run()`, repeated `JAVAN030` lambda `invokedynamic`, and unsupported JDK calls such as `Map.computeIfAbsent`, `Optional.filter/ifPresent`, `ConcurrentHashMap.<init>()`, and `Object.getClass()`. | Real lambda/functional dispatch, broader lambda `invokedynamic`, deterministic JDK substitutions/collections for the Nano bootstrap chain, then broader HTTP service runtime, resources, thread/blocking model, and dev-console/reflection exclusion. |
...
- stripped real Nano `/hello` service route now gets past inherited `Nano.subscribeEvent(...)->NanoBase` resolution, inherited JDK `Context.computeIfAbsent(..., Function)` classification, inherited TypeMap default-interface false positives, the hidden-lambda default wrapper false positive on `FunctionOrNull.apply(Object)`, and the inherited-JDK `Context.put(...)->ConcurrentTypeMap.put(...)` false positive; its current deterministic blocker set is pinned in reports as `FunctionOrNull.applyWithException(Object)`, `ExRunnable.run()`, repeated lambda `invokedynamic`, and missing JDK substitutions such as `Map.computeIfAbsent`, `Optional.filter/ifPresent`, `ConcurrentHashMap.<init>()`, and `Object.getClass()` before HTTP runtime behavior can be claimed
