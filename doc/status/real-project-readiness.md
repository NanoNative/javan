...
| Nano HTTP service | In progress | A stripped real Nano `/hello` service route without `DevConsoleService` now gets past inherited `Nano.subscribeEvent(...)->NanoBase` resolution and inherited JDK `Context.computeIfAbsent(..., Function)` classification, then reproduces the next current blocker deterministically against pinned Nano + TypeMap artifacts: `error[JAVAN012]` on `FunctionOrNull.apply(Object)`. | Closed-world interface/lambda dispatch for `FunctionOrNull` and the surrounding Nano bootstrap chain, then broader HTTP service runtime, resources, thread/blocking model, and dev-console/reflection exclusion. |
...
- stripped real Nano `/hello` service route now gets past inherited `Nano.subscribeEvent(...)->NanoBase` resolution and inherited JDK `Context.computeIfAbsent(..., Function)` classification, and currently stops at `error[JAVAN012]` on `FunctionOrNull.apply(Object)`, so the next M11 gate is closed-world interface/lambda dispatch through the Nano bootstrap chain before HTTP runtime behavior can be claimed
...
6. in progress: stripped Nano `/hello` route without `DevConsoleService` now reproduces the current blocker deterministically; inherited `Nano.subscribeEvent(...)->NanoBase` resolution and inherited JDK `Context.computeIfAbsent(..., Function)` classification are fixed locally, and the next gate is `JAVAN012` on `FunctionOrNull.apply(Object)` before the HTTP runtime
