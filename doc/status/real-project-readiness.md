...
- the current main-based Nano frontier has moved through the scheduler/bootstrap worker slice plus the Nano service bootstrap `AtomicBoolean` subset: `ScheduledThreadPoolExecutor.<init>(int)`, `ScheduledThreadPoolExecutor.<init>(int,ThreadFactory,RejectedExecutionHandler)`, same-method static virtual-thread builder/factory bootstrap, runtime ownership for `Scheduler extends ScheduledThreadPoolExecutor`, `AtomicLong(long|get|incrementAndGet|decrementAndGet)`, `AtomicBoolean.<init>()`, `AtomicBoolean.<init>(boolean)`, `AtomicBoolean.get()`, `ExecutorService.submit(Runnable)`, `Future.cancel(boolean)`, `schedule(...)`, `scheduleAtFixedRate(...)`, `awaitTermination(...)`, and `shutdownNow()` are now integrated and acceptance-backed; the next concrete Nano blocker is reachable non-string `invokedynamic` on the HTTP-service path, first observed at `org/nanonative/nano/helper/config/ConfigRegister.registerConfig(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;`, with `CopyOnWriteArrayList` and `CountDownLatch` immediately behind it on the service-thread path
