...
| External HTTP service smoke | Partial smoke | Compiler-owned public-entrypoint tests now prove one-context `com.sun.net.httpserver.HttpServer` bind/context/start/request/response/stop parity and actual POST `HttpExchange.getRequestMethod()` dispatch against the JVM; the pinned external HTTP-service-shaped probe still fails clearly with `JAVAN061` and reports `network/http`. | Broader external HTTP service runtime, routing/request semantics, resources, thread/blocking model, and dev-console/reflection exclusion. |
