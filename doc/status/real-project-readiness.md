...
| External HTTP service smoke | Partial smoke | Compiler-owned public-entrypoint tests now prove one-context `com.sun.net.httpserver.HttpServer` bind/context/start/request/response/stop parity, actual POST `HttpExchange.getRequestMethod()` dispatch, and one-byte `HttpExchange.getRequestBody()` delivery against the JVM; the pinned external HTTP-service-shaped probe still fails clearly with `JAVAN061` and reports `network/http`. | Broader external HTTP service runtime, routing/body framing/request semantics, resources, thread/blocking model, and dev-console/reflection exclusion. |
