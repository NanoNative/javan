...
| Nano HTTP service | In progress | A stripped real Nano `/hello` service route without `DevConsoleService` now gets past inherited `Nano.subscribeEvent(...)->NanoBase` resolution, inherited JDK `Context.computeIfAbsent(..., Function)` classification, and TypeMap default-interface false positives. The current deterministic blocker set is now pinned in tests and reports: `JAVAN012` on `FunctionOrNull.apply(Object)` and `ExRunnable.run()`, repeated `JAVAN030` lambda `invokedynamic`, and unsupported JDK calls such as `Map.computeIfAbsent`, `Optional.filter/map`, and `Object.getClass()`. | Real lambda/functional dispatch, broader lambda `invokedynamic`, deterministic JDK substitutions/collections for the Nano bootstrap chain, then broader HTTP service runtime, resources, thread/blocking model, and dev-console/reflection exclusion. |
...
- stripped real Nano `/hello` service route now gets past inherited `Nano.subscribeEvent(...)->NanoBase` resolution, inherited JDK `Context.computeIfAbsent(..., Function)` classification, and TypeMap default-interface false positives; its current deterministic blocker set is pinned in reports as `FunctionOrNull.apply(Object)`, `ExRunnable.run()`, repeated lambda `invokedynamic`, and missing JDK substitutions such as `Map.computeIfAbsent`, `Optional.filter/map`, and `Object.getClass()` before HTTP runtime behavior can be claimed
...
6. in progress: stripped Nano `/hello` route without `DevConsoleService` now reproduces the current blocker set deterministically; inherited `Nano.subscribeEvent(...)->NanoBase` resolution, inherited JDK `Context.computeIfAbsent(..., Function)` classification, and TypeMap default-interface false positives are fixed locally, and the current pinned gates are `FunctionOrNull.apply(Object)`, `ExRunnable.run()`, repeated lambda `invokedynamic`, and missing JDK substitutions before the HTTP runtime
