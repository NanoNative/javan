...
| --- | ---: | --- |
| Self-check reachable classes | 223 | Clean detached branch verification of `java -cp target/classes javan.Main check target/classes --main javan.Main` on current committed Javan classes. |
| Self-check reachable methods | 2,535 | Same clean detached branch self-check. |
...
| Exact supported JDK callables | 982/267886 (0.3%) | Lower-bound callable-member coverage on the scanned JDK 25 image for members that already match the exact native support registry on the clean committed branch baseline. |
...
| Exact done JDK callables | 6017/267886 (2.2%) | Lower-bound supported-plus-explicitly-rejected callable-member accounting on the scanned JDK 25 image for the clean committed branch baseline. |
| Exact unknown JDK callables | 261869 | Callable members on the scanned JDK 25 image not yet counted as supported or explicitly rejected. |
| Exact supported JDK callables left | 266904 | Callables on the scanned JDK 25 image that are not yet in the exact supported callable ledger. |
...
| Supported JDK accounting | In progress | Compatibility docs and support matrix exist; `compatibility-summary.*`, `support-matrix.*`, and `javan report` expose the current scenario ledger (`108` rows, `107` pass, `1` target), an exact supported JDK callable ledger for the clean committed JDK 25 baseline (`457` classes with at least one supported callable, `678` supported constructors, `304` supported methods, `982 / 267886` callables, `0.3%`), and an exact callable accounting baseline widened by deterministic verifier-backed rejects plus deliberate unsupported owner families: `5035` explicit rejected callables, `6017 / 267886` done callables (`2.2%`), and `261869` unknown callables. The current M22, M23, M24, M25, and queued M26 parity slices added exact support for `String.valueOf(int)`, `Paths.get(String,String...)`, boolean `Arrays.copyOf`, floating `Math.abs(float)` and `Math.abs(double)`, primitive `String.valueOf(long|float|double|boolean|char)`, `PrintStream.print(char|boolean|int|long|float|double)`, `PrintStream.println(char)`, and `StringBuilder.append(float|double)`, all verified through native build or lowering coverage. Flow-qualified rejected JDK call shapes are now reported separately from the member denominator with family counts for current-thread lifecycle, thread-builder receiver shape, virtual-thread factory shape, and executor receiver shape; the current self-check profile has `0` such diagnostics. Reachable JDK reports also split current reachable call sites into intrinsic, runtime-registry, supported-direct, and unsupported buckets, and `check` keeps failing reachable unsupported JDK calls while still writing that ledger. `typemap-pair`, `nano-metric`, and `nano-duration` are now promoted from target to pass because the pinned real-probe gate proves exact stdout plus `diagnostics: 0`. `try-finally` remains the only named target row in the scenario ledger. Remaining work is broader explicit rejected member coverage and eventual `unknown = 0` for a release-gated JDK. |
