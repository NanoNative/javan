...
| --- | ---: | --- |
| Self-check reachable classes | 226 | `java -cp target/classes javan.Main check target/classes --main javan.Main` on current Javan classes. |
| Self-check reachable methods | 2,662 | Same self-check. |
...
| Exact done JDK callables | 2456/267886 (0.9%) | Lower-bound supported-plus-explicitly-rejected callable-member accounting on the scanned JDK 25 image. |
| Exact unknown JDK callables | 265430 | Callable members on the scanned JDK 25 image not yet counted as supported or explicitly rejected. |
...
| Supported JDK accounting | In progress | Compatibility docs and support matrix exist; `compatibility-summary.*`, `support-matrix.*`, and `javan report` expose the current scenario ledger (`108` rows, `107` pass, `1` target), an exact supported JDK callable ledger for the scanned JDK 25 image (`459` classes with at least one supported callable, `778` supported constructors, `287` supported methods, `1065 / 267886` callables, `0.3%`), and an exact callable accounting baseline widened by deterministic verifier-backed rejects: `1391` explicit rejected callables, `2456 / 267886` done callables (`0.9%`), and `265430` unknown callables. Reachable JDK reports also split current reachable call sites into intrinsic, runtime-registry, supported-direct, and unsupported buckets, and `check` keeps failing reachable unsupported JDK calls while still writing that ledger. `typemap-pair`, `nano-metric`, and `nano-duration` are now promoted from target to pass because the pinned real-probe gate proves exact stdout plus `diagnostics: 0`. `try-finally` remains the only named target row in the scenario ledger. Remaining work is broader explicit rejected member coverage, flow-qualified rejected accounting, and eventual `unknown = 0` for a release-gated JDK. |
