...
| Roadmap rows with implementation evidence | 24 | 38 | 63.2% | Rows marked `Done`, `Partial`, `In progress`, or `Blocked`. |
...
1. `Platform threads`: interrupted `Thread.sleep(long)` and `Thread.join()` now
   ...
   gate is virtual-thread scheduler/carrier runtime plus broader builder-object
   flows, then runtime-backed profiling counters and broader reachable
   virtual-thread API accounting, rather than more host-thread bootstrap
   groundwork.
2. `Virtual threads`: `Thread.startVirtualThread(Runnable)` plus the direct
   ...
   `println(Object)`, `toString()`, `hashCode()`, and `equals()` now also
   compiles natively for the current supported slice. Broader introspection such
   as `getClass()` still fails clearly instead of pretending to work.
   ...
   host-thread-backed slice. The next virtual-thread-specific gate is remaining
...
3. `GC` / `Managed heap`: finish thread-root-aware heap correctness so self-build and
