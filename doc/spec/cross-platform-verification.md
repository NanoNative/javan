...
| --- | --- | --- |
| linux/amd64 Wolfi | required release image | Docker Buildx on `ubuntu-24.04` |
| linux/arm64 Wolfi | required release image | Docker Buildx on native `ubuntu-24.04-arm` |
| linux/amd64 distroless | required release image | Docker Buildx on `ubuntu-24.04` |
| linux/arm64 distroless | required release image | Docker Buildx on native `ubuntu-24.04-arm` |
| linux/amd64 scratch | required release image | Docker Buildx on `ubuntu-24.04` |
| linux/arm64 scratch | required release image | Docker Buildx on native `ubuntu-24.04-arm` |
...
Each native image build runs `javan --version`. A separate manifest job combines the
architecture-specific images and verifies that each pushed public tag contains `linux/amd64`
and `linux/arm64`; it does not use emulation. The default Wolfi image
...
- Docker Linux aarch64 passes on a native Linux ARM64 runner
