...
Status: Done for the currently enabled release package targets. Remote package-backed
self-host sanitizer proof passed on Linux x64, Linux ARM64, and macOS ARM64 with nonzero
tracked allocation/GC counters, zero final live heap/root residue, and no sanitizer
failure signatures. The reduced `platform-smoke` path reuses the just-generated self-host
C output and narrows repeated probes to `--version` plus a tiny build/check loop while
retaining the counter and residue assertions. macOS x64 and Windows package rows remain
explicitly disabled and are not release support claims.
...
latest accepted remote matrix run produced nonzero tracked self-host allocations and GC
