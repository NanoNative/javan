...
if [ -z "$ARCHIVE" ] || [ ! -f "$ARCHIVE" ]; then
  printf '%s\n' "Usage: scripts/verify-package.sh dist/release/javan-<version>-<os>-<arch>.tar.gz" >&2
  exit 2
fi

case "$ARCHIVE" in
  *SNAPSHOT*)
    printf '%s\n' "Release archive must not contain SNAPSHOT: $ARCHIVE" >&2
    exit 1
    ;;
esac
...
case "$VERSION" in
  *SNAPSHOT*|"")
    printf '%s\n' "Release version must not be empty or SNAPSHOT: $VERSION" >&2
    exit 1
    ;;
esac
